  ...
  with(patch('src.generator._execute_requests', return_value=[{'text': 'テスト回答', 'status': 'success'} for _ in range(len(QUERIES_LIST[0]))]) as mock_execute):
    ...
    assert generate(client, SYSTEM_PROMPT, QUERIES_LIST, row_count) == (
      [
        ...
        {'conversation_id': '3', 'query': 'テスト質問', 'text': 'テスト回答', 'status': 'success'}, {'conversation_id': '3', 'query': 'テスト質問', 'text': 'テスト回答', 'status': 'success'},
        {'conversation_id': '4', 'query': 'テスト質問', 'text': 'テスト回答', 'status': 'success'}, {'conversation_id': '4', 'query': 'テスト質問', 'text': 'テスト回答', 'status': 'success'},
        {'conversation_id': '5', 'query': 'テスト質問', 'text': 'テスト回答', 'status': 'success'}, {'conversation_id': '5', 'query': 'テスト質問', 'text': 'テスト回答', 'status': 'success'},
        {'conversation_id': '6', 'query': 'テスト質問', 'text': 'テスト回答', 'status': 'success'}, {'conversation_id': '6', 'query': 'テスト質問', 'text': 'テスト回答', 'status': 'success'},
      ],
      0
    )
...
def test_generate_with_api_error():
  with(patch('src.generator._execute_requests') as mock_execute):
    mock_execute.side_effect = [
      [{'text': 'テスト回答', 'status': 'success'} for _ in range(len(QUERIES_LIST[0]))],
    ...
    assert generate(client, SYSTEM_PROMPT, QUERIES_LIST, row_count) == (
      [
        ...
        {'conversation_id': '3', 'query': 'テスト質問', 'text': 'テスト回答', 'status': 'success'}, {'conversation_id': '3', 'query': 'テスト質問', 'text': 'テスト回答', 'status': 'success'},
        {'conversation_id': '4', 'query': 'テスト質問', 'text': '', 'status': 'API_ERROR'}, {'conversation_id': '4', 'query': 'テスト質問', 'text': '', 'status': 'API_ERROR'},
        {'conversation_id': '5', 'query': 'テスト質問', 'text': 'テスト回答', 'status': 'success'}, {'conversation_id': '5', 'query': 'テスト質問', 'text': 'テスト回答', 'status': 'success'},
        {'conversation_id': '6', 'query': 'テスト質問', 'text': '', 'status': 'API_ERROR'}, {'conversation_id': '6', 'query': 'テスト質問', 'text': '', 'status': 'API_ERROR'},
      ],
      3
    )
    mock_execute.assert_has_calls(calls=[
...
def test_execute_requests_openai_success(monkeypatch):
  ...
  with (
    patch('src.generator.request_openai', return_value={'text': 'テスト回答', 'status': 'success'}) as mock_request,
...
def test_execute_requests_ollama_success(monkeypatch):
  ...
  monkeypatch.setenv('REQUEST_INTERVAL_TIME', '5.0')
  with (
    patch('src.generator.request_ollama', return_value={'text': 'テスト回答', 'status': 'success'}) as mock_request,
    patch('src.generator.time.sleep') as mock_sleep
  ):
    mock_request = copy_call_args(mock_request)
    client = 'ollama'
